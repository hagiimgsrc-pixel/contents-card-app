...
const contents = [
  { id: 1, title: "Business Class", artist: "DARIO", serial: "1456", image: "https://images.unsplash.com/photo-1614613535308-eb5fbd3d2c17?w=300&h=300&fit=crop" },
  { id: 2, title: "悠久のアルカナ", artist: "久牧彰", serial: "3122", image: "https://images.unsplash.com/photo-1493225255756-d9584f8606e9?w=300&h=300&fit=crop" },
  { id: 3, title: "SUNDANCE", artist: "PLOP", serial: "457", image: "https://images.unsplash.com/photo-1459749411177-042180ce673c?w=300&h=300&fit=crop" },
];
...
const LoadingScreen = ({ onComplete }: { onComplete: () => void }) => {
    ...
    const animate = (ts: number) => {
      ...
      if (progress < 1) {
        ...
        setTimeout(() => { 
          ...
          if (checkWrapRef.current) {
            checkWrapRef.current.style.transform = 'translate(-50%, -50%) scale(1)';
            checkWrapRef.current.style.opacity = '1';
          }
  ...
  return (
    <div style={{ background: "#0a0a0a", minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "40px", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ color: "rgba(255,255,255,0.5)", fontSize: "14px" }}><span ref={dlPercentRef}>0%</span></div>
  ...
  const SearchHeader = () => (
    <div style={{ padding: "16px 24px", backgroundColor: "#fff", display: "flex", alignItems: "center", gap: "16px", borderBottom: "1px solid #f0f0f0", position: "sticky", top: 0, zIndex: 10 }}>
      <div style={{ flex: 1, display: "flex", alignItems: "center", gap: "10px", backgroundColor: "#f3f4f6", padding: "10px 16px", borderRadius: "25px" }}>
        <span style={{ color: "#999" }}><Search size={18} /></span>
    ...
    <>
      ...
      {view === "login" && (
        <main style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px" }}>
          <div style={{ fontSize: "32px", fontWeight: "bold", letterSpacing: "0.1em", marginBottom: "40px" }}>VENU.</div>
          ...
          <div style={{ padding: "24px" }}>
            ...
            {contents.map((item) => (
              <div key={item.id} onClick={() => { setSelected(item); setView("details"); }} style={{ display: "flex", alignItems: "center", gap: "16px", marginBottom: "20px" }}>
                <img src={item.image} alt={item.title} style={{ width: "60px", height: "60px", borderRadius: "8px", boxShadow: "0 4px 12px rgba(0,0,0,0.1)", objectFit: "cover" }} />
                <div>
      ...
      {view === "details" && (
        <main style={{ minHeight: "100vh", paddingBottom: "100px" }}>
          <SearchHeader />
          <div style={{ padding: "24px" }}>
            <ArrowLeft onClick={() => setView("list")} style={{ marginBottom: "20px", cursor: "pointer" }} />
            <div style={{ textAlign: "center" }}>
              <img src={selected.image} alt={selected.title} style={{ width: "100%", maxWidth: "280px", borderRadius: "16px", boxShadow: "0 20px 40px rgba(0,0,0,0.15)", objectFit: "cover" }} />
      ...
      {view === "profile" && (
        <main style={{ minHeight: "100vh" }}>
          <div style={{ display: "flex", alignItems: "center", padding: "16px 24px", borderBottom: "1px solid #f0f0f0" }}>
            <ArrowLeft onClick={() => setView("list")} style={{ marginRight: "16px", cursor: "pointer" }} />
          ...
          <div style={{ padding: "0 24px" }}>
            {[
              ...
              <div key={i} onClick={item.action} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 0", borderBottom: "1px solid #fcfcfc", cursor: "pointer" }}>
      ...
      {view === "echoes" && (
        <main style={{ minHeight: "100vh", backgroundColor: "#f9f9f9" }}>
          <div style={{ display: "flex", alignItems: "center", padding: "16px 24px", backgroundColor: "#fff", borderBottom: "1px solid #f0f0f0" }}>
            <ArrowLeft onClick={() => setView("profile")} style={{ marginRight: "16px", cursor: "pointer" }} />
